...
def importdata(): 
    balance_data = pd.read_csv('C:/Users/Geeta/Desktop/Use of Artificial Neural Networks to Identify FakeProfiles/Profile/dataset/dataset.txt')
...
def UserCheck(request):
    if request.method == 'POST':
      ...
      input+=data+"\n"
      f = open("C:/Users/Geeta/Desktop/Use of Artificial Neural Networks to Identify FakeProfiles/Profile/dataset/test.txt", "w")
      f.write(input)
      f.close()
      test = pd.read_csv('C:/Users/Geeta/Desktop/Use of Artificial Neural Networks to Identify FakeProfiles/Profile/dataset/test.txt')
      test = test.values[:, 0:8]
      graph = tf.get_default_graph()
      with graph.as_default():
          predict = model.predict_classes(test)
          #predict = model._make_predict_function(test)
          print(predict[0])
          msg = ''
          if str(predict[0]) == '0':
...
def GenerateModel(request):
    ...
    print(model.summary())
    model.fit(train_x, train_y, verbose=2, batch_size=5, epochs=95)
...
def ViewTrain(request):
    if request.method == 'GET':
       ...
       data = pd.read_csv('C:/Users/Geeta/Desktop/Use of Artificial Neural Networks to Identify FakeProfiles/Profile/dataset/dataset.txt')
